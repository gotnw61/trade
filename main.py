...
from wallet import wallet_manager, get_available_balance, async_input, WalletManager
...
from core.websocket_manager import start_enhanced_websocket, add_token_subscription
    ...
    try:
        ...
        if "wallets" in state and "active_wallet_index" in state:
            ...
            try:
                ...
                logger.error(f"Cüzdan yükleme ana hatası:Systemd[2196]: [main] INFO: Durum dosyası başarıyla okundu.")
...
async def start_bot(console_mode=True, gui_mode=False):
    ...
    try:
        from config import AI_MODEL_FILE
    ...
    if trade_settings["rapid_cycle_enabled"]:
        ...
        tasks.append(asyncio.create_task(start_rapid_cycle(trade_bot)))

    # WebSocket başlat
    tasks.append(asyncio.create_task(start_enhanced_websocket(trade_bot)))
    await add_token_subscription(trade_bot, "So11111111111111111111111111111111111111112")
